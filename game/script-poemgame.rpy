...
label poem(transition=True):
    ...
    if persistent.playthrough == 3:
        ...
        if not persistent.sayori_dead:
            show s_sticker at sticker_left
        if not persistent.natsuki_dead:
            show n_sticker at sticker_mid
        if not persistent.yuri_dead:
    ...
    python:
                ...
                if t.glitch:
                    ...
                    if persistent.playthrough == 0:
                        ...
                        if persistent.natsuki_dead == 1: 
                            if t.sPoint > t.yPoint:
                                renpy.show("s_sticker hop")
                            else:
                                renpy.show("y_sticker hop")
                        elif persistent.yuri_dead == 1: 
                            if t.sPoint > t.nPoint:
                                renpy.show("s_sticker hop")
                            else:
                                renpy.show("n_sticker hop")
                        elif persistent.sayori_dead == 1: 
                            if t.nPoint > t.yPoint:
                                renpy.show("n_sticker hop")
                            else:
                                renpy.show("y_sticker hop")
            else:
        ...
        if persistent.playthrough == 0:
            ...
            elif persistent.natsuki_dead == 1:
                if sPointTotal > yPointTotal: poemwinner[chapter] = "sayori"
                else: poemwinner[chapter] = "yuri"
            else:
                if sPointTotal > nPointTotal: poemwinner[chapter] = "sayori"
                else: poemwinner[chapter] = "natsuki"
