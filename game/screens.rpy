...
screen main_menu() tag menu:
    ...
    add "menu_bg"
    if not persistent.yuri_dead == 1:
        add "menu_art_y"
    if not persistent.natsuki_dead == 1:
        add "menu_art_n"
    if not persistent.sayori_dead == 1:
        add "menu_art_s"
    frame
...
init python:
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            if persistent.natsuki_dead == 1: # Using ch4_scene variables doesn't work
                return Show(screen="dialog", message="File error: \"characters/natsuki.chr\"\n\nThe file is missing or corrupt.",
                ...
                return Show(screen="dialog", message="File error: \"characters/yuri.chr\"\n\nThe file is missing or corrupt.",
                ok_action=Show(screen="dialog", message="The save file is corrupt. Starting a new game.", ok_action=Function(renpy.full_restart, label="start")))
            elif persistent.sayori_dead == 1:
                return Show(screen="dialog", message="File error: \"characters/sayori.chr\"\n\nThe file is missing or corrupt.",
                ok_action=Show(screen="dialog", message="The save file is corrupt. Starting a new game.", ok_action=Function(renpy.full_restart, label="start")))
            else:
                return Show(screen="dialog", message="File error",
                ok_action=Show(screen="dialog", message="The save file is corrupt. Starting a new game.", ok_action=Function(renpy.full_restart, label="start")))
...
screen confirm(message, yes_action, no_action):
    ...
    frame:
        ...
        if in_character_death and message == layout.QUIT:
